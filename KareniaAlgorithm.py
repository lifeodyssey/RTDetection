...
# Bbp can be calculated using either Carder et al. (1999) or the QAA algorithm (Lee et al., 2002).
    ...
    gs.plot_geo_image(Imagebbp,lons,lats,log10=False,title='Result by Cannizzaro et al(2004,2008,2009)'+nc_file.time_coverage_end[0:13],caxis=[0,6],save_image='True')
...
# A negative SS_490 is indicative of a K. brevis bloom.
    ...
    nlw443=Rrs_443*F0[1]
    nlw531=Rrs_531*F0[4]
    SS_90 = nlw488-nlw443-(nlw531-nlw443)*((488-443)/(531443))
...
# indicates a dinoflagellate bloom but not specifically a K. brevis bloom.
    ...
    gs.plot_geo_image(ImageRBD, lons, lats, log10=False,
                      title='Result by Amin, Gilerson et al(2009)' + '\n' + nc_file.time_coverage_end[0:13],
                      caxis=[0, 6], save_image='True')
...
# How much is chla affected?
    ...
    for i in range(len(y)):
        for j in range(len(x)):
            if(nlw547[j,i]>max(nlw412[j,i],nlw443[j,i],nlw469[j,i],nlw488[j,i],nlw531[j,i],nlw555[j,i],nlw645[j,i],nlw667[j,i],nlw678[j,i])):
                if(not(nlw443_412slope[j,i]>nlw488_443slope[j,i])&(nlw488_443slope[j,i]>nlw547_488slope[j,i])):
                    if(not(nlw547_412slope[j,i])>-0.0003*((np.log(chl[j,i]))**2+0.0024*np.log(chl[j,i]))-0.00005):
                        if(not(nlw547[j,i]>0.8)):
                            if(not(abs(nlw547_488slope[j,i])-abs(nlw488_412slope[j,i])>0.006)):
                                ImageEKO[j,i]=5
